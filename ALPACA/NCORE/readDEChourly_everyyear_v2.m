[StateCode,CountyCode, SiteID, Parameter, POC, latitude, longitude,	datum, parameter, date_local, time_local, date_gmt, time_gmt, SampleValue, units_of_measure, sample_duration, sample_frequency, detection_limit, uncertainty,...
    ...
    readvars('/Volumes/GoogleDrive/My Drive/group_member/current/Ragen/2019data/DEC_data/hourlydata_2013_2019/NCOREhourly_2013.csv');
...
[SO2_time,sort_indx]=sort(SO2_time_unsort);
SO2=SO2_unsort(sort_indx);

indx=find(StateCode==02 & CountyCode ==90 & SiteID ==034 & Parameter==42101);
CO_unsort=SampleValue(indx);
CO_time_unsort=convertDOY(strcat(datestr(date_local(indx),'yyyymmdd'),char(time_local(indx))));
%try to sort the arry
[CO_time,sort_indx]=sort(CO_time_unsort);
CO=CO_unsort(sort_indx);

indx=find(StateCode==02 & CountyCode ==90 & SiteID ==034 & Parameter==42600);
NOy_unsort=SampleValue(indx);
NOy_time_unsort=convertDOY(strcat(datestr(date_local(indx),'yyyymmdd'),char(time_local(indx))));
%try to sort the arry
[NOy_time,sort_indx]=sort(NOy_time_unsort);
NOy=NOy_unsort(sort_indx);
...
O3_unsort=SampleValue(indx);
O3_time_unsort=convertDOY(strcat(datestr(date_local(indx),'yyyymmdd'),char(time_local(indx))));
%try to sort the arry
[O3_time,sort_indx]=sort(O3_time_unsort);
O3=O3_unsort(sort_indx);
...
t3m_unsort=SampleValue(indx);
t3m_time_unsort=convertDOY(strcat(datestr(date_local(indx),'yyyymmdd'),char(time_local(indx))));
%try to sort the arry
[t3m_time,sort_indx]=sort(t3m_time_unsort);
t3m=t3m_unsort(sort_indx);
...
pm25_unsort=SampleValue(indx);
pm25_time_unsort=convertDOY(strcat(datestr(date_local(indx),'yyyymmdd'),char(time_local(indx))));
%try to sort the arry
[pm25_time,sort_indx]=sort(pm25_time_unsort);
pm25=pm25_unsort(sort_indx);

save NCOREdata2013.mat CO CO_time SO2 SO2_time NOy NOy_time O3 O3_time ...
