[state_code,county_code, site_number, parameter_code, poc, latitude, longitude,	datum, parameter, date_local, time_local, date_gmt, time_gmt, sample_measurement, units_of_measure, sample_duration, sample_frequency, detection_limit, uncertainty,...
...
indx=find(state_code==02 & county_code ==90 & site_number ==034 & parameter_code==42401& poc==1);
SO2=sample_measurement(indx);
SO2_time=convertDOY(date_local(indx),time_local(indx));

indx=find(StateCode==02 & CountyCode ==90 & SiteID ==034 & Parameter==42101);
CO=SampleValue(indx);
CO_time=convertDOY(date_local(indx),time_local(indx));

indx=find(StateCode==02 & CountyCode ==90 & SiteID ==034 & Parameter==42600);
NOy=SampleValue(indx);
NOy_time=convertDOY(date_local(indx),time_local(indx));
...
O3=SampleValue(indx);
O3_time=convertDOY(date_local(indx),time_local(indx));
...
t3m_time=convertDOY(date_local(indx),time_local(indx));
...
pm25_time=convertDOY(date_local(indx),time_local(indx));

save NCOREdata2013.mat CO CO_time SO2 SO2_time NOy NOy_time O3 O3_time wind10m_spd wind10m_spd_time wind3m_spd wind3m_spd_time wind10m_dir wind3m_dir_time ...
    t10m t10m_time t3m t3m_time rh rh_time pm25 pm25_time

function doy=convertDOY (tmp1,tmp2)
for i=1:length(tmp1)
    temp=inputtime(i,:);
