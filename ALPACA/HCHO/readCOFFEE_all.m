clear all;
tempdir = '/Volumes/GoogleDrive/My Drive/ALPACA/COFFEE/ICARTT';
filelist = dir(fullfile(tempdir, '**', 'HCHO_Alaska2020_*.ict'));
...
for fileid=1:length(filename_new)
    
[UTC_seconds, HCHO_pptv]=textread(char(filename_new(fileid)),'%f %f','delimiter', ',','headerlines',33);
display(char(filename_new(fileid)));
filename_tmp=filelist(fileid).name;
day=julian(filename_tmp(21:24));
doy=day+UTC_seconds./3600./24;
doy=doy-9/24;%correct for UTC
tempdir2='/Volumes/GoogleDrive/My Drive/ALPACA/COFFEE/analysis/';
%here I used end-4 to remove last four digit .ict. Also there were two
%files on Jan 19.
datafile=[tempdir2 filename_tmp(1:end-4) '.mat']; 
save (datafile, 'doy', 'HCHO_pptv');
end
