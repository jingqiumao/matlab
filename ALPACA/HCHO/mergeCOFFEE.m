clear all;
tempdir = '/Volumes/GoogleDrive/My Drive/ALPACA/COFFEE/analysis/';
filelist = dir(fullfile(tempdir, '**', 'HCHO_Alaska2020_*.mat'));
for fileid=1:length(filelist)
    filename(fileid) = cellstr([filelist(fileid).folder '/' filelist(fileid).name]);
end
%sort the file names
filename_new = natsortfiles(filename);

    load(char(filename_new(1)));
    display(char(filename_new(1)));
    eval(['doy1 = doy; HCHO_pptv1=HCHO_pptv; ']);

    load(char(filename_new(2)));
    display(char(filename_new(2)));
    eval(['doy2 = doy; HCHO_pptv2=HCHO_pptv; ']);
    eval(['doy_mrg = [doy1;doy2]; HCHO_pptv_mrg=[HCHO_pptv1;HCHO_pptv2];'])
    
for fileid=2:length(filename_new)
    load(char(filename_new(fileid)));
    display(char(filename_new(fileid)));
    eval(['doy3 = doy; HCHO_pptv3=HCHO_pptv; ']);
    eval(['clear doy HCHO_pptv;']);
    eval(['doy_mrg = [doy_mrg;doy3]; HCHO_pptv_mrg=[HCHO_pptv_mrg;HCHO_pptv3];'])
    eval(['clear HCHO_pptv3 doy3;']);
end
    eval(['save mergeHCHO_1s.mat doy_mrg HCHO_pptv_mrg'])
