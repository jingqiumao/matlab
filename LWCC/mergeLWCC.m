clear all;
tempdir = '/Volumes/GoogleDrive/Team Drives/Data/LWCC_new/July Data/';
filelist = dir(fullfile(tempdir, '**', 'LWCC2018*.mat'));
...
for fileid=2:length(filename_new)
    ...
    eval(['doy1 = doy; ab3651=ab365; ab6001=ab600; ab7001=ab700; ']);
    eval(['clear toc doy;']);
    load mergeTemp.mat
    eval(['doy = [doy doy1]; ab365=[ab365 ab3651]; ab600=[ab600 ab6001]; ab700=[ab700 ab7001];'])
    eval(['clear toc1 doy1;']);
    eval(['save mergeLWCC.mat doy ab365 ab600 ab700'])
end
...
%         if ~strcmp(tempstr(j),'fileid')==1 & ~strcmp(tempstr(j),'j')==1 & ~strcmp(tempstr(j),'tempstr')==1 & ~strcmp(tempstr(j),'tempdir')==1 & ~strcmp(tempstr(j),'filename')==1 & ~strcmp(tempstr(j),'filelist')==1& ~strcmp(tempstr(j),'str')==1
